...
export const multipleImageUploadService = async (files: Express.Multer.File[], id: string) => {
  try {
      ...
      files.map(
        (file) =>
          new Promise((resolve, reject) => {
            cloudinary.uploader
              .upload_stream({ folder: 'e-commerce/products' }, (error, result) => {
    ...
    const productFind = await Product.findByIdAndUpdate(
      id,
      { $set: { images, thumbnail: firstImage } },
      {
        new: true,
      },
    )
